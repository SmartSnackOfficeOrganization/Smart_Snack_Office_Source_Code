...
class BuyerRegistrationSerializer(PasswordValidationMixin, RegistrationValidationMixin, serializers.Serializer):
    """
...
class SellerRegistrationSerializer(PasswordValidationMixin, RegistrationValidationMixin, serializers.Serializer):
    """
    Serializer for seller registration.
    Creates a User with 'seller' role and a SellerProfile.
    """
    email = serializers.EmailField()
    full_name = serializers.CharField(max_length=200, help_text="Owner name or representative name")
    business_name = serializers.CharField(max_length=200)
    password = serializers.CharField(write_only=True)
    confirm_password = serializers.CharField(write_only=True)
    terms_accepted = serializers.BooleanField()
    tax_info = serializers.CharField(max_length=500, required=False, allow_blank=True)
    commercial_info = serializers.CharField(max_length=500, required=False, allow_blank=True)
